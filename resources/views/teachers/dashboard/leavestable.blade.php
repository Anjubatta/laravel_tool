<div class="tile-body">
	<div class="table-responsive">
		<table class="table">
			...
			<tbody>
				@foreach($teacher_leaves as $i => $leave)
				<tr>					
					<td>{{ $leave->leaves_names->name}}</td>
					<td>{{ $leave->leaves_types->type}}</td>
					<?php if(!empty($leave->from_date) && !empty($leave->to_date)) { ?>
					<td>{{ $leave->from_date}} {{ @$leave->from_time}}</td>
					<td>{{ $leave->to_date}} {{ @$leave->to_time}}</td>
				<?php } ?>
